...
public class AppController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		col_c1.setCellValueFactory(new PropertyValueFactory<Dato, String>("c1"));
		 ...
		 //tv_tablaDatos.getColumns().addAll(col_c1, col_c2);
		 
		 
		 // Inicialización del repdroductor de video


		 Media media = new Media(getClass().getResource("video_informatica.mp4").toExternalForm());
		 media_reproduccion.setMediaPlayer(new MediaPlayer(media));
	        // Configurar el reproductor para que se reproduzca de forma automática (opcional)
		 media_reproduccion.getMediaPlayer().setAutoPlay(true);
	 	}
    ...
    void oa_BotonGuardarTabla(ActionEvent event) {
		...
		System.out.println("Ciudad 2: " + tf_c2.getText());
		 ...
		 col_c2.setCellValueFactory(new PropertyValueFactory<Dato, String>("c2"));
		 tv_tablaDatos.getColumns().addAll(col_c1, col_c2);*/
		tv_tablaDatos.getItems().add(new Dato(tf_c1.getText(), tf_c2.getText()));
		tv_tablaDatos.refresh();
		
    }
    
    @FXML
    void omc_tabla(MouseEvent event) {
		if (event.getClickCount() == 2) {
			Dato d = tv_tablaDatos.getSelectionModel().getSelectedItem();
			System.out.println("C1: " + d.getC1());
			System.out.println("C2: " + d.getC2());
		}
    }
}
