...
public class Aplicacion {
	...
	public static void main(String[] args) {
		...
		JCheckBox jcbOpcion1 = new JCheckBox("visualizar contraseña");
		jcbOpcion1.addActionListener((e) -> {
			jpf_contrasena.setEchoChar(((JCheckBox) e.getSource()).isSelected() ? (char) 0 : '*');
		});
		constraintLayout.gridx=0;
		constraintLayout.gridy=3;
		constraintLayout.gridwidth=2;
		constraintLayout.fill = GridBagConstraints.HORIZONTAL;
		jpf_contrasena.setAlignmentX(JPasswordField.LEFT_ALIGNMENT);
		panelEste.add(jcbOpcion1,constraintLayout);
		
		
	
		String[] datos = {"Elemento 1", "Elemento 2", "Elemento 3", "Elemento 4"};
		JList<String> jlist = new JList<String>(datos); 
		//jlist.setListData(new String[] {"Elemento 1", "Elemento 2", "Elemento 3", "Elemento 4"});
		jframe.getContentPane().add(jlist, BorderLayout.CENTER);
		jlist.setBorder(new TitledBorder("Listado"));
		...
		botonEnviar.addActionListener((e) -> {
			...
			int elementosSeleccionados[]=jlist.getSelectedIndices();
			for (int i = 0; i < elementosSeleccionados.length; i++) {
				System.out.println("Elemento seleccionado: " + datos[elementosSeleccionados[i]]);
			}
		});
