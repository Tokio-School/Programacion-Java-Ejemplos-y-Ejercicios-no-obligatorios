...
public class Ejem03_ArrayBidimensionales {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i <= comunidadesProvincias.length-1; i++) {
		...
		// Array bidimensional de diferentes tamaños
		String[][] arrayDiferente=new String[10][];
		for (int i = 0; i < arrayDiferente.length; i++) {
			arrayDiferente[i]=new String[i];
			Arrays.fill(arrayDiferente[i],0,arrayDiferente[i].length," . ");
		}

		for (int i = 0; i < arrayDiferente.length; i++) {
			for (int j = 0; j < arrayDiferente[i].length; j++) {
				System.out.print(arrayDiferente[i][j]);
			}
			System.out.println();
		}
